...
{
	bool MainPage::DisplaySettings_ADL_Convert_ToUI(DisplaySettings displaySettings)
	{
		try
		{
			//HDR Enabled
	...
	bool MainPage::DisplaySettings_Profile_Convert_ToUI(DisplaySettings displaySettings)
	{
		try
		{
			//Brightness
	...
	std::optional<DisplaySettings> MainPage::DisplaySettings_Generate_FromUI()
	{
		try
		{
			...
			{
				displaySettings.Brightness.Current = slider_Display_Brightness().Value();
			}

			//Contrast
			if (slider_Display_Contrast().IsEnabled())
			{
				displaySettings.Contrast.Current = slider_Display_Contrast().Value();
			}

			//Saturation
			if (slider_Display_Saturation().IsEnabled())
			{
				displaySettings.Saturation.Current = slider_Display_Saturation().Value();
			}

			//Hue
			if (slider_Display_Hue().IsEnabled())
			{
				displaySettings.Hue.Current = slider_Display_Hue().Value();
			}

			//Color Temperature Control
			if (toggleswitch_Display_ColorTemperature_Control().IsEnabled())
			{
				displaySettings.ColorTemperatureControl.Current = toggleswitch_Display_ColorTemperature_Control().IsOn();
			}

			//Color Temperature Kelvin
			if (slider_Display_ColorTemperature_Kelvin().IsEnabled())
			{
				displaySettings.ColorTemperatureKelvin.Current = slider_Display_ColorTemperature_Kelvin().Value();
			}

			//Color Deficiency Correction Control
			if (toggleswitch_Display_CVDC_Control().IsEnabled())
			{
				displaySettings.CVDCControl.Current = toggleswitch_Display_CVDC_Control().IsOn();
			}

			//Color Deficiency Correction Protanopia
			if (slider_Display_Protanopia().IsEnabled())
			{
				displaySettings.CVDCProtanopia.Current = slider_Display_Protanopia().Value();
			}

			//Color Deficiency Correction Deuteranopia
			if (slider_Display_Deuteranopia().IsEnabled())
			{
				displaySettings.CVDCDeuteranopia.Current = slider_Display_Deuteranopia().Value();
			}

			//Color Deficiency Correction Tritanopia
			if (slider_Display_Tritanopia().IsEnabled())
			{
				displaySettings.CVDCTritanopia.Current = slider_Display_Tritanopia().Value();
			}
